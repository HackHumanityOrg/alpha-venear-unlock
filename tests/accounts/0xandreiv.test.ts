/**
 * Test: 0xandreiv.near - Already unlocking flow (no staking)
 *
 * Account State (from mainnet):
 * - Owner: 0xandreiv.near
 * - Lockup: c126bf21a3e354662f41c78ee9d66aa361a653aa.v.voteagora.near
 * - Locked: 0 NEAR
 * - Pending: ~0.10 NEAR (ALREADY UNLOCKING)
 * - Staking: None
 *
 * Test Flow:
 * 1. Import lockup contract from mainnet (already has pending balance)
 * 2. Create impersonated owner account
 * 3. Patch unlock_duration_ns to 1 second (so unlock completes immediately)
 * 4. fast_forward(200 blocks) to ensure unlock timestamp passes
 * 5. end_unlock_near({ amount: null }) - convert pending to liquid
 * 6. transfer({ amount, receiver_id })
 * 7. delete_lockup()
...
const OWNER_ACCOUNT = "0xandreiv.near";
const LOCKUP_CONTRACT = "c126bf21a3e354662f41c78ee9d66aa361a653aa.v.voteagora.near";
...
test.serial(`${OWNER_ACCOUNT}: complete unlock flow`, async (t) => {
  ...
  // Step 2: Patch unlock timestamp to 0 (so unlock is immediately available)
  t.log(`\nStep 2: Patching unlock_timestamp to 0...`);
  await patchUnlockTimestamp(lockup, 0n);

  // Step 3: Fast forward to ensure any time-based checks pass
  t.log(`\nStep 3: Fast forwarding 200 blocks...`);
  const ffStart = Date.now();
  await worker.provider.fastForward(200);
  t.log(`Fast forwarded in ${Date.now() - ffStart}ms`);

  // Step 4: End unlock (convert pending to liquid)
  t.log(`\nStep 4: End unlock (pending → liquid)...`);
