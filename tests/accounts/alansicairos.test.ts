/**
 * Test: alansicairos.near - Staking unlock flow (with Meta Pool + epoch wait)
 *
 * Account State:
 * - Owner: alansicairos.near
 * - Lockup: 6a4f205c5062e79edbc10e54b3f818b7a7632517.v.voteagora.near
 * - Locked: ~4.85 NEAR
 * - Staking Pool: meta-pool.near
 *
 * Test Flow:
...
const OWNER_ACCOUNT = "alansicairos.near";
const LOCKUP_CONTRACT = "6a4f205c5062e79edbc10e54b3f818b7a7632517.v.voteagora.near";
...
test("alansicairos.near: Staking unlock flow with epoch wait", async (t) => {
  // Set a long timeout for this test (20 minutes)
  t.timeout(1200000);

  // Import contracts (no beforeEach to avoid hook timeout issues)
  ...
  try {
    ...
    await importContractFromCustomRpc(root, VENEAR_FACTORY);
    ...
    // Import staking pool
    t.log("Importing Meta Pool staking pool...");
    ...
    // Step 2: Fast forward 4 epochs (172,800 blocks) in chunks to avoid timeout/memory issues
    t.log(
    ...
    const CHUNK_SIZE = 10000; // Fast forward in 10k block chunks
    const chunks = Math.ceil(TOTAL_BLOCKS / CHUNK_SIZE);

    for (let i = 0; i < chunks; i++) {
      const blocksThisChunk = Math.min(CHUNK_SIZE, TOTAL_BLOCKS - i * CHUNK_SIZE);
      t.log(`  Chunk ${i + 1}/${chunks}: forwarding ${blocksThisChunk.toLocaleString()} blocks...`);
      await worker.provider.fastForward(blocksThisChunk);
    }
